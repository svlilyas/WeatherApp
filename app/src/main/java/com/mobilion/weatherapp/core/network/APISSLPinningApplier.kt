package com.mobilion.weatherapp.core.network

import android.content.res.AssetManager
import android.net.Uri
import android.text.TextUtils
import com.mobilion.weatherapp.BuildConfig
...
class APISSLPinningApplier : SSLPinningApplier {
    ...
    override fun apply(clientBuilder: OkHttpClient.Builder, assetManager: AssetManager, certificatePaths: List<String>) {
        ...
        try {
            ...
            Timber.d("Certificate did not add for %s", com.mobilion.weatherapp.BuildConfig.FLAVOR)
        } catch (e: Exception) {
            Timber.w(e)
        }
    }
    ...
    fun getSSLPublicKeys(): List<String> {
        if (com.mobilion.weatherapp.BuildConfig.DEBUG) return emptyList()
        if (TextUtils.isEmpty(com.mobilion.weatherapp.BuildConfig.SERVICE_PUBLIC_KEY)) return emptyList()
        return com.mobilion.weatherapp.BuildConfig.SERVICE_PUBLIC_KEY.split(",")
    }
}
